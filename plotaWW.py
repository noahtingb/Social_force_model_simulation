...
def load_json(filename="variance_of_speed//variance_of_speed_v2.json"):
...
for k, samples in enumerate(samples_array):
    if true_array[k]:
        ax.errorbar(stds_[:musk], all_mean_ps[k,:musk], all_std_ps[k,:musk],alpha=1, color="tab:red",fmt='o', capsize=6, capthick=2)
        ax.plot(stds_, y1_exp, '--', color='black', alpha=1, label='fit non-linear', linewidth=2)

        ax.plot(stds_[:musk1+1], m0+stds_[:musk1+1]*c0, '--', color='tab:blue', alpha=1, label='linear-fit $\mathrm{\sigma_v\leq0.28}$', linewidth=2)
        ax.plot(stds_[musk1:musk], m1+stds_[musk1:musk]*c1, '--', color='tab:orange', alpha=1, label='linear-fit $\mathrm{\sigma_v\geq0.28}$', linewidth=2)
        #ax.plot(stds_, y2_exp, '--', color='black', alpha=1, label='fit exponential', linewidth=2)
        ax.scatter(stds_[:musk], all_mean_ps[k,:musk], s=10, color='tab:red', label=f"data")


ax.set_ylabel("$\mathrm{\dfrac{<|<v_x>|_i>}{<|v_{max}|_i>}}$")
ax.set_xlabel("$\mathrm{\sigma_v}$  [$\mathrm{\dfrac{m}{s}}$]")
