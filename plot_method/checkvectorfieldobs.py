...
ax.set_title('In an environment with an obstacle',fontsize=11)
cbar = plt.colorbar(q, cax=cax)
cbar.set_label('Acceleration [$\mathrm{m\,s^{-2}}$]', fontsize=11)
ticks=[0.2, 0.5, 1.0, 2, 5, 9.82]
cbar.ax.yaxis.set_major_locator(FixedLocator(ticks))
cbar.ax.set_yticklabels([f"{t:g}" for t in ticks],fontsize=11)  # fixed labels
...
plt.tight_layout()
plt.savefig("C://Users//46738//OneDrive//Dokument//VS-program chalmers//SOCS_project//plot_method//method obs.png", dpi=300, bbox_inches="tight")
plt.show()
