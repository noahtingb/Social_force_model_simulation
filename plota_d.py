...
for samples in samples_array:
    ...
    partspeed = (52/times_a)/np.abs(max_speed_a)
    partspeed_global = (52/times_a)/mean_ms_0
...
c0, m0 = np.polyfit(obst, all_mean_ps[0,:], 1)
...
for k, samples in enumerate(samples_array):
    if true_array[k]:
        ...
        print(obst.shape,all_mean1_ps.shape)
        ax.scatter(np.repeat(obst[:, np.newaxis], 10, axis=1), all_mean1_ps[k,:,:],  color='tab:blue', label=f"10 sample from 10 simulations")
